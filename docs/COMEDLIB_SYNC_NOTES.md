...
## Bugs fixed in battery-control copy (need to port upstream)

### 1. Infinite loops on network failure

`getMostRecentRate()` and `getPredictedRate()` used `while data is None:`
which hangs forever if `downloadComedJsonData()` returns None. The `while`
was an intentional retry mechanism but has no attempt limit or backoff.

**Fix**: changed to a bounded retry loop (3 attempts) with `return None`
if all attempts fail. Note that `downloadComedJsonData()` already has its
own internal retries via `safeDownloadWebpage()` (up to 9 attempts with
exponential backoff), so 3 outer retries gives ~27 total download attempts
before giving up.
